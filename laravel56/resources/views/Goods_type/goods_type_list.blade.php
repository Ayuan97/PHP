...
<h1>
    <span class="action-span"><a href="{{URL('Admin/Goods_type/goods_type_add')}}">新建商品类型</a></span>
    ...
    <div class="list-div" id="listDiv">
        ...
        <table width="100%" cellpadding="3" cellspacing="1" id="listTable">
            ...
            <tr data-id="{{$val->cat_id}}">
                ...
                <td align="center" class="status" val="{{$val->status}}">
                    @if($val->status == 1)
                    <img src="{{URL::asset('images/yes.gif')}}">
                    @else
                   <img src="{{URL::asset('images/no.gif')}}">
                    @endif
                </td>
                <td align="center">
                    <a href="attribute.php?act=list&amp;goods_type=1" title="属性列表" value="{{$val->cat_id}}">属性列表</a> |
                    <a href="{{URL('Admin/Goods_type/goods_type_edit')}}?cat_id={{$val->cat_id}}" title="编辑">编辑</a> |
            ...
            <tr>
                <td align="right" nowrap="true" colspan="6" style="background-color: rgb(255, 255, 255);">
                    ...
                    <div id="turn-page">
                        ...
                        <span id="page-link">
          <a href="{{URL('Admin/Goods_type/goods_type_list')}}?page=1">第一页</a>
          <a href="{{URL('Admin/Goods_type/goods_type_list')}}?page=<?= max($page-1,1)?>">上一页</a>
          <a href="{{URL('Admin/Goods_type/goods_type_list')}}?page=<?= min($page+1,$sum)?>">下一页</a>
          <a href="{{URL('Admin/Goods_type/goods_type_list')}}?page= <?= $sum?>">最末页</a>
...
<script>
    ...
    $('.status').click(function () {
        ...
        $.ajax({
            ...
            success:function (e) {
                $(".status").attr('val').val(e);1
                $(".status").img("src","{{URL::asset('images/no.gif')}}");
            }
        })
    })
